...
const elf = catalog.find((hunt) => hunt.name === "Elf");
const quantities = elf.req.map((entry) => entry.qty);
const almostDone = quantities.map((quantity, index) => index === quantities.length - 1 ? 11 : quantity);
const progress = core.codexCompletion(quantities, almostDone);
assert.equal(progress.percent, 99);
assert.equal(progress.remaining.at(-1), 4);
assert.equal(progress.ready, false);
assert.equal(core.codexCompletion(quantities, quantities).percent, 100);
